...
class Dataloader(pl.LightningDataModule):
    def __init__(self, model_name, batch_size, shuffle, train_path, dev_path, test_path, predict_path, is_test=False):
        ...
        self.max_length = 160
        self.tokenizer = KoBERTTokenizer.from_pretrained(model_name, sp_model_kwargs={'nbest_size': -1, 'alpha': 0.6, 'enable_sampling': True})
    ...
    def tokenizing(self, df_input):
        ...
        for idx, item in tqdm(df_input.iterrows(), desc='tokenizing', total=len(df_input)):
            ...
            #     text, add_special_tokens=True, padding='max_length', truncation=True) #기존버전
            outputs = self.tokenizer.encode_plus(str(text), add_special_tokens=True, max_length=self.max_length, pad_to_max_length='longest', truncation=True, return_attention_mask=True, return_tensors='pt')
            
            # data_input.append(outputs['input_ids']) #기존버전
            data_input.append(outputs)
...
class Model(pl.LightningModule):
    def __init__(self, model_name, lr):
        ...
        #     pretrained_model_name_or_path=model_name, num_labels=1) #기존버전
        self.bert = BertModel.from_pretrained(model_name, output_attentions=True)
        self.W = nn.Linear(self.bert.config.hidden_size, 1)
